...
MACRO(EXCLUDE target_list regexp)
    SET(_new_list "")
    SET(_regex "^.*${regexp}.*")
    MESSAGE(STATUS "EXCLUDE      : \"${_regex}\"")
    FOREACH(_curr_path ${${target_list}})
        IF(NOT ${_curr_path} MATCHES "${_regex}")
            SET(_new_list "${_new_list};${_curr_path}")
        ELSE()
...
MACRO(FIND_CMAKE_LIBS result_list target_dir)
    ...
    LIST(LENGTH _cmake_paths_copy _cmake_length)
    SET(_cmake_i 0)
    WHILE(${_cmake_i} LESS ${_cmake_length})
        LIST(GET _cmake_paths_copy ${_cmake_i} _curr_path)
        get_filename_component(_cur_dir ${_curr_path} PATH)
        LIST(APPEND _cmake_dirs ${_cur_dir})
        EXCLUDE_DIRS(_cmake_paths_copy ${_cur_dir})
        LIST(LENGTH _cmake_paths_copy _cmake_length)
        MATH(EXPR _cmake_i "${_cmake_i} + 1" OUTPUT_FORMAT DECIMAL)
    ENDWHILE()
